describe('API Login Testing', () => {
  ...
  it('log in via API and verify user details ', () => {

    cy.loginViaApi(Cypress.env("username"), Cypress.env("password"))
    cy.getUserDetails().then(() => {
      cy.get('@username').then((username) => {       //... get username from auth token and fetch user details
        cy.log(`Logged-in Username: ${username}`)
      });
    });
  });
});
